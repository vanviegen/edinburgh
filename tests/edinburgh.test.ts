...
const Person = E.defineModel("Person", class {
    ...
    pk: "name",
    unique: { combi: ["name", "test"], car: "cars" },
});

const Simple = E.defineModel("Simple", class {
...
const Data = E.defineModel("Data", class {
    id = E.field(E.identifier, {description: "Unique identifier"});
...
const CompositeKeyModel = E.defineModel("CompositeKeyModel", class {
...
test("Boolean type validation", async () => {
    const Settings = E.defineModel("Settings", class {
...
test("Model persistence", async () => {
    const TestModel = E.defineModel("TestModel", class {
        id = E.field(E.number);
...
test("Update a lazy-loaded row", async () => {
    ...
    await E.transact(async () => {
        const simple = Simple.findBy("value", {is: 1234}).fetch();
    ...
    await E.transact(async () => {
        const simple = Simple.findBy("value", {is: 4321}).fetch();
        expect(simple).toBeDefined();
        expect(simple!.value).toBe(4321);
...
test("Advanced model lifecycle and registration", async () => {
    ...
    expectErrorCode("INIT_ERROR", () => {
        E.defineModel("User", class {
            id = E.field(E.identifier);
        });
    });

    // Test model with complex dependencies
    const ModelA = E.defineModel("ModelA", class {
        id = E.field(E.identifier);
        bRef = E.field(E.opt(E.link(() => ModelB)), {description: "Reference to B"});
    }, { pk: "id", override: true });

    const ModelB = E.defineModel("ModelB", class {
        id = E.field(E.identifier);
...
test("Index system comprehensive", async () => {
    ...
    await E.transact(() => {
        const foundByValue = CompositeKeyModel.getBy("value", 999);
...
test("Model state management and persistence", async () => {
    ...
    await E.transact(() => {
        ...
        loaded = User.getBy("email", "state@test.com");
        expect(loaded).toBeDefined();
        expect(loaded!.name).toBe("State Test");
    });
...
test("Field validation comprehensive", async () => {
    const ValidationTest = E.defineModel("ValidationTest", class {
        id = E.field(E.identifier);
...
test("Default value handling advanced", async () => {
    const DefaultTest = E.defineModel("DefaultTest", class {
        id = E.field(E.identifier);
...
test("Modification tracking and proxy behavior", async () => {
    ...
    await E.transact(() => {
        const loaded = User.getBy("email", "track@test.com");
        expect(loaded).toBeDefined();
...
test("Error handling and recovery", async () => {
    ...
    const ErrorTest = E.defineModel("ErrorTest", class {
        id = E.field(E.identifier);
    ...
    await E.transact(() => {
        const user = User.getBy("email", "error@test.com");
...
test("Database operations and debugging", async () => {
    ...
    await E.transact(() => {
        const user = User.getBy("email", "dump@test.com");
    ...
    await E.transact(() => {
        for (let i = 0; i < 5; i++) {
            const user = User.getBy("email", `concurrent${i}@test.com`);
            expect(user).toBeDefined();
...
test("Range queries on unique indices", async () => {
    function testRange() {
        let names: string[] = [];
        const users = User.findBy("email", {from: "b@test.com", to: "f@test.com"});
    ...
    function testExclusiveRange() {
        let names: string[] = [];
        const users = User.findBy("email", {from: "b@test.com", before: "f@test.com"});
        for(const user of users) {
            names.push(user.name);
        }
    ...
    await E.transact(() => {
        ...
        const users = User.findBy("email", {to: "d@test.com"});
        for(const user of users) {
    ...
    await E.transact(() => {
        let toEnd: string[] = [];
        const users = User.findBy("email", {from: "f@test.com"});
        for(const user of users) {
    ...
    await E.transact(() => {
        ...
        const users = User.findBy("email", {is: "d@test.com"});
        for(const user of users) {
...
test("Secondary index implementation", async () => {
    ...
    // This should not compile: Product.findBy("price", {is: 1000});
    ...
    await E.transact(() => {
        ...
        for (const product of Product.findBy("price", {from: 500})) {
        ...
        for (const product of Product.findBy("price", {from: 200, to: 600})) {
        ...
        for (const product of Product.findBy("category", {is: "electronics"})) {
        ...
        for (const product of Product.findBy("stock", {is: false})) {
    ...
    await E.transact(() => {
        ...
        for (const product of Product.findBy("categoryPrice", {from: ["electronics"], to: ["electronics", 700]})) {
        ...
        for (const product of Product.findBy("categoryPrice", {from: ["furniture"], to: ["furniture", Number.MAX_SAFE_INTEGER]})) {
        ...
        for (const product of Product.findBy("categoryPrice", {is: ["books"]})) {
    ...
    await E.transact(() => {
        ...
        for (const product of Product.findBy("price", {reverse: true})) {
    ...
    await E.transact(() => {
        let cnt = 0;
        for (const product of Product.findBy("price", {is: 800})) {
            product.price--;
            cnt++;
        }
        expect(cnt).toBe(1);
    ...
    await E.transact(() => {
        // expect(Product.getBy("name", "Phone")!.price).toBe(799);
        let count = 0;
        for (const product of Product.findBy("price", {from: 790, to: 810})) {
        ...
        expect(Product.findBy("categoryPrice", {is: ["electronics", 799]}).count()).toBe(1);
        expect(Product.findBy("categoryPrice", {is: ["electronics", 800]}).count()).toBe(0);
    ...
    await E.transact(() => {
        ...
        for (const product of Product.findBy("price", {is: 600})) {
    ...
    await E.transact(() => {
        ...
        for (const product of Product.findBy("price", {after: 500, before: 1000})) {
    ...
    await E.transact(() => {
        ...
        for (const product of Product.findBy("category", {is: "nonexistent"})) {
            noResults.push(product.name);
        }
        expect(noResults).toEqual([]);

        const noResultsRange: string[] = [];
        for (const product of Product.findBy("price", {from: 2000, to: 3000})) {
...
test("onSave callback with transaction rollback", async () => {
    ...
    await E.transact(() => {
        expect(User.getBy("email", "rollback@test.com")).toBeUndefined();
    });
});
...
test("onSave callback with unique constraint failures", async () => {
    ...
    await E.transact(() => {
        const user = User.getBy("email", "unique@test.com");
        expect(user).toBeDefined();
...
test("onSave callback with multiple models and operations", async () => {
    ...
    await E.transact(() => {
        const user = User.get(userId);
        const post = Post.findBy("author", {is: [user!, "Test Post"]}).fetch();
    ...
    await E.transact(() => {
        const user = User.get(userId);
        const post = Post.findBy("author", {is: [user!, "Updated Test Post"]}).fetch();
        post!.delete();
...
test("preCommit() is called before writes", async () => {
    const PreCommitModel = E.defineModel("PreCommitModel", class {
        id = E.field(E.identifier);
        name = E.field(E.string);
...
test("lazy migration adds a new non-key field", async () => {
    ...
    const LazyMigV1 = E.defineModel("LazyMigrateTest", class {
        id = E.field(E.string);
        name = E.field(E.string);
    }, { pk: "id" });
    ...
    const LazyMigV2 = E.defineModel("LazyMigrateTest", class {
        id = E.field(E.string);
    ...
    const LazyMigV3 = E.defineModel("LazyMigrateTest", class {
        id = E.field(E.string);
        name = E.field(E.string);
        role = E.field(E.string, {default: "user"});
    }, { pk: "id", override: true });
...
test("runMigration converts rows from old primary key format", async () => {
    ...
    const PkConvertV1 = E.defineModel("PkConvertTest", class {
        id = E.field(E.string);
    }, { pk: "id" });
    ...
    const PkConvertV2 = E.defineModel("PkConvertTest", class {
        id = E.field(E.number);
...
test("runMigration populates new secondary indexes", async () => {
    ...
    const SecPopV1 = E.defineModel("SecondaryPopTest", class {
        id = E.field(E.string);
    ...
    const SecPopV2 = E.defineModel("SecondaryPopTest", class {
        id = E.field(E.string);
        category = E.field(E.string);
        score = E.field(E.number, {default: 0});
    }, {
    ...
    await E.transact(() => {
        const xItems = [...SecPopV2.findBy("category", {is: "x"})];
        expect(xItems.length).toBe(2);
        expect(xItems.map((i: any) => i.id).sort()).toEqual(["a", "c"]);

        const yItems = [...SecPopV2.findBy("category", {is: "y"})];
...
test("runMigration fixes secondaries affected by migrate()", async () => {
    ...
    const SecMigV1 = E.defineModel("SecMigrateTest", class {
        id = E.field(E.string);
    ...
    await E.transact(() => {
        expect([...SecMigV1.findBy("tag", {is: "old"})].length).toBe(1);
        expect([...SecMigV1.findBy("tag", {is: "keep"})].length).toBe(1);
    });

    // Step 2: Register V2 (override V1) with a migrate() that changes indexed tag values
    const SecMigV2 = E.defineModel("SecMigrateTest", class {
        id = E.field(E.string);
        tag = E.field(E.string);
    ...
    await E.transact(() => {
        expect([...SecMigV2.findBy("tag", {is: "old"})].length).toBe(0);
        expect([...SecMigV2.findBy("tag", {is: "new"})].length).toBe(1);
        expect([...SecMigV2.findBy("tag", {is: "new"})][0].id).toBe("a");
        expect([...SecMigV2.findBy("tag", {is: "keep"})].length).toBe(1);
    });
});

test("runMigration removes orphaned secondary index entries", async () => {
    // Step 1: Register V1 with a secondary index and write data
    const OrphanV1 = E.defineModel("OrphanedSecTest", class {
        id = E.field(E.string);
        tag = E.field(E.string);
    }, {
        pk: "id",
    ...
    const OrphanV2 = E.defineModel("OrphanedSecTest", class {
        id = E.field(E.string);
        tag = E.field(E.string);
    }, { pk: "id", override: true });
    ...
    await E.transact(() => {
        expect(OrphanV1.findBy("tag", {is: "x"}).count()).toBe(1);
        expect(OrphanV1.findBy("tag", {is: "y"}).count()).toBe(1);
    });
    ...
    await E.transact(() => {
        expect(OrphanV1.findBy("tag", {is: "x"}).count()).toBe(0);
        expect(OrphanV1.findBy("tag", {is: "y"}).count()).toBe(0);
    });
...
test("preCommit() can create new instances", async () => {
    const Child = E.defineModel("PreCommitChild", class {
        parentId = E.field(E.string);
        data = E.field(E.string);
    }, { pk: "parentId" });

    const Parent = E.defineModel("PreCommitParent", class {
        id = E.field(E.identifier);
        name = E.field(E.string);
        preCommit() {
...
test("runMigration reports no work needed for clean data", async () => {
    const CleanModel = E.defineModel("CleanModel", class {
        id = E.field(E.string);
...
test("Version info rows are created in the database", async () => {
    const VersionInfoModel = E.defineModel("VersionInfoModel", class {
        id = E.field(E.string);
...
test("find() with fetch: 'first' returns first or undefined", async () => {
    ...
    await E.transact(() => {
        ...
        const none = Simple.findBy("value", {is: 999, fetch: 'first'});
...
test("find() with fetch: 'single' returns exactly one or throws", async () => {
    ...
    await E.transact(() => {
        const one = Simple.findBy("value", {is: 42, fetch: 'single'});
        expect(one.value).toBe(42);
    });

    // No results: throws
    await E.transact(() => {
        expectErrorCode("NOT_FOUND", () => Simple.findBy("value", {is: 999, fetch: 'single'}));
    });
...
test("batchProcess respects range options via secondary index", async () => {
    ...
    const seen: number[] = [];
    await Simple.batchProcessBy("value", { from: 3, to: 7, limitRows: 2 }, (row) => {
        seen.push(row.value);
    });
...
test("Record type stores and retrieves key-value objects", async () => {
    const Config = E.defineModel("Config", class {
        name = E.field(E.string);
...
test("Record type with numeric keys", async () => {
    const NumKeyModel = E.defineModel("NumKeyModel", class {
        id = E.field(E.identifier);
...
test("Record type defaults to empty object", async () => {
    const RecDefault = E.defineModel("RecDefault", class {
        id = E.field(E.identifier);
...
test("Record type validates values", async () => {
    const RecValidation = E.defineModel("RecValidation", class {
        id = E.field(E.identifier);
...
test("computed unique index: basic CRUD and get", async () => {
    const Employee = E.defineModel("Employee", class {
        id = E.field(E.identifier);
    ...
    await E.transact(() => {
        const john = Employee.getBy("fullName", "John Doe");
        expect(john).toBeDefined();
        expect(john!.firstName).toBe("John");
        expect(john!.lastName).toBe("Doe");

        const jane = Employee.getBy("fullName", "Jane Smith");
        expect(jane).toBeDefined();
        expect(jane!.firstName).toBe("Jane");

        expect(Employee.getBy("fullName", "Nobody Here")).toBeUndefined();
    });
});

test("computed unique index: unique constraint violation", async () => {
    const UniqueComputed = E.defineModel("UniqueComputed", class {
        id = E.field(E.identifier);
        a = E.field(E.string);
        b = E.field(E.string);
    }, {
...
test("computed secondary index: basic find", async () => {
    const Product = E.defineModel("Product2", class {
        id = E.field(E.identifier);
    ...
    await E.transact(() => {
        ...
        const bucket0 = [...Product.findBy("priceBucket", {is: 0})];
        expect(bucket0.length).toBe(1);
        expect(bucket0[0].price).toBe(50);

        // Find all products in the $100-$199 bucket
        const bucket1 = [...Product.findBy("priceBucket", {is: 1})];
        expect(bucket1.length).toBe(2);

        // Range query
        const buckets01 = [...Product.findBy("priceBucket", {from: 0, to: 1})];
        expect(buckets01.length).toBe(3);

        // Find all
        const all = [...Product.findBy("priceBucket")];
        expect(all.length).toBe(4);
    });
});

test("computed index: undefined return skips indexing (partial index)", async () => {
    const MaybeIndexed = E.defineModel("MaybeIndexed", class {
        id = E.field(E.identifier);
    ...
    await E.transact(() => {
        ...
        const all = [...MaybeIndexed.findBy("activePriority")];
        expect(all.length).toBe(2);

        const p1 = [...MaybeIndexed.findBy("activePriority", {is: 1})];
...
test("computed index: updates re-index on any field change", async () => {
    const Updatable = E.defineModel("Updatable", class {
        id = E.field(E.identifier);
        x = E.field(E.number);
        y = E.field(E.number);
    }, {
    ...
    await E.transact(() => {
        // Sum is 7
        const found7 = [...Updatable.findBy("sum", {is: 7})];
    ...
    await E.transact(() => {
        ...
        const found10 = [...Updatable.findBy("sum", {is: 10})];
        expect(found10.length).toBe(1);
    });
});

test("computed index: delete removes index entries", async () => {
    const Deletable = E.defineModel("Deletable", class {
        id = E.field(E.identifier);
        tag = E.field(E.string);
    }, {
        pk: "id",
    ...
    await E.transact(() => {
        expect([...Deletable.findBy("tag", {is: "foo"})].length).toBe(2);

        // Delete one
        const first = [...Deletable.findBy("tag", {is: "foo"})][0];
        first.delete();
    });

    await E.transact(() => {
        expect([...Deletable.findBy("tag", {is: "foo"})].length).toBe(1);
        expect([...Deletable.findBy("tag", {is: "bar"})].length).toBe(1);
    });
});

test("computed unique index: find with range and reverse", async () => {
    const Scored = E.defineModel("Scored", class {
        id = E.field(E.identifier);
        name = E.field(E.string);
        score = E.field(E.number);
    }, {
        pk: "id",
        unique: { score: (s: any) => [s.score] },
    });
...
test("computed index: multi-value indexing", async () => {
    const Article = E.defineModel("Article", class {
        ...
        pk: "id",
        index: { word: (a: any) => a.title.toLowerCase().split(" ") },
    });
    ...
    await E.transact(() => {
        expect([...Article.findBy("word", {is: "hello"})].length).toBe(2);
        expect([...Article.findBy("word", {is: "world"})].length).toBe(2);
        expect([...Article.findBy("word", {is: "there"})].length).toBe(1);
        expect([...Article.findBy("word", {is: "goodbye"})].length).toBe(1);
    });

    // Update "Hello There" → "Greetings Everyone" (pick via "there" to be deterministic)
    await E.transact(() => {
        const target = [...Article.findBy("word", {is: "there"})][0];
        target.title = "Greetings Everyone";
    });

    await E.transact(() => {
        expect([...Article.findBy("word", {is: "hello"})].length).toBe(1);
        expect([...Article.findBy("word", {is: "there"})].length).toBe(0);
        expect([...Article.findBy("word", {is: "greetings"})].length).toBe(1);
        expect([...Article.findBy("word", {is: "everyone"})].length).toBe(1);
        expect([...Article.findBy("word", {is: "world"})].length).toBe(2); // unchanged
    });

    // Delete the "Greetings Everyone" article
    await E.transact(() => {
        const greet = [...Article.findBy("word", {is: "greetings"})][0];
        greet.delete();
    });

    await E.transact(() => {
        expect([...Article.findBy("word", {is: "greetings"})].length).toBe(0);
        expect([...Article.findBy("word", {is: "everyone"})].length).toBe(0);
        expect([...Article.findBy("word", {is: "world"})].length).toBe(2);
    });
});
